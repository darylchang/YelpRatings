...
class RNN:
    ...
    def forwardProp(self,node,correct=[], guess=[]):
        ...
        # Base case
        if node.isLeaf:
            wordvec = np.maximum(self.defaultVec(), self.L[:, node.word])
            node.probs = self.softmax(self.Ws.dot(wordvec) + self.bs)
            node.hActs1 = wordvec
            node.fprop = True
            cost = -np.log(node.probs[node.label])
        else:
            # Calculate y_hat
            leftCost, leftTotal = self.forwardProp(node.left, correct, guess)
            rightCost, rightTotal = self.forwardProp(node.right, correct, guess)
            children = np.hstack((node.left.hActs1, node.right.hActs1))
            node.hActs1 = np.maximum(self.defaultVec(), self.W.dot(children) + self.b)
            node.probs = self.softmax(self.Ws.dot(node.hActs1) + self.bs)
            node.fprop = True
            currCost = -np.log(node.probs[node.label])
            cost = leftCost + rightCost + currCost
            total = leftTotal + rightTotal

        return cost, total + 1

    def makeOneHot(self,i,n):
        y = np.zeros(n)
        y[i] = 1
        return y

    # def getErrors(self,node,error=None):
    #     if node.isLeaf:
    #         derivative = 1 #np.sign(self.L[:,node.word])
    #     else:
    #         children = np.hstack((node.left.hActs1, node.right.hActs1))
    #         derivative = np.sign(self.W.dot(children) + self.b)
    #         np.place(derivative, derivative==-1, 0)

    #     delta_yhat = node.probs - self.makeOneHot(node.label, self.outputDim) 
    #     y_term = self.Ws.T.dot(delta_yhat) * derivative

    #     delta_h = 0.
    #     if node.parent is None:
    #         delta_h = y_term
    #     elif node.parent.left is node:
    #         delta_h = self.W[:,:self.wvecDim].T.dot(error) * derivative + y_term
    #     elif node.parent.right is node:
    #         delta_h = self.W[:,self.wvecDim:].T.dot(error) * derivative + y_term

    #     return delta_yhat, delta_h, derivative
    ...
    def backProp(self,node,error=None):
        ...
        ################
        deltas = node.probs - self.makeOneHot(node.label, self.outputDim) 
        self.dWs += np.outer(deltas, node.hActs1)
        self.dbs += deltas

        deltas = self.Ws.T.dot(deltas)
        ...
        # delta_yhat, delta_h, derivative = self.getErrors(node, error)
        # self.dWs += np.outer(delta_yhat, node.hActs1)
        # self.dbs += delta_yhat

        # if node.parent:
        #     self.db += delta_h
        #     if node.parent.left is node:
        #         self.dW[:, :self.wvecDim] += np.outer(error, node.hActs1)
        #     elif node.parent.right is node:
        #         self.dW[:, self.wvecDim:] += np.outer(error, node.hActs1)

        # if node.isLeaf:
        #     self.dL[node.word] += delta_h
        # else:
        #     self.backProp(node.left, delta_h[:self.wvecDim])
        #     self.backProp(node.right, delta_h[self.wvecDim:])
