...
class RNN2:
    ...
    def forwardProp(self,node, correct=[], guess=[]):
        ...
        # Base case
        if node.isLeaf:
            wordvec = self.L[:, node.word]
            node.hActs1 = wordvec
            node.hActs2 = np.maximum(np.zeros(self.middleDim), self.W2.dot(node.hActs1) + self.b2)
            ...
            node.hActs1 = np.maximum(self.defaultVec(), self.W1.dot(children) + self.b1)
            node.hActs2 = np.maximum(np.zeros(self.middleDim), self.W2.dot(node.hActs1) + self.b2)
            node.probs = self.softmax(self.Ws.dot(node.hActs2) + self.bs)
            node.fprop = True
    ...
    def backProp(self,node,error=None):
        ...
        if node.isLeaf:
            ...
            if node.parent.left is node:
                dL_update = self.W1[:,:self.wvecDim].T.dot(error) + self.W2.T.dot(deltas)
            elif node.parent.right is node:
                dL_update = self.W1[:,self.wvecDim:].T.dot(error) + self.W2.T.dot(deltas)
            dL_update *= (self.L[:,node.word] != 0)
            self.dL[node.word] += dL_update 
            return 
        else:
            self.dW2 += np.outer(deltas, node.hActs1)
            self.db2 += deltas
            ...
            # Delta_one
            deltas = self.W2.T.dot(deltas)
            if node.parent:
                deltas += error

            deltas *= (node.hActs1 != 0)
            children = np.hstack((node.left.hActs1, node.right.hActs1))
            self.dW1 += np.outer(deltas, children)
            self.db1 += deltas
